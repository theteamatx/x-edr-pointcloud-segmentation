#include "googlex/proxy/object_properties/point_cloud/plane_classification.h"
...
bool IsHorizontalPlaneWithOffset(
  ...
  const float cos_max_up_direction_delta_angle = std::cos(
      MathUtil::DegToRad(params.max_up_direction_delta_angle_degrees()));
...
bool IsWall(const ClassifyWallParams& params,
            const PlanarRegion<>& planar_region,
            const eigenmath::Vector3f& up_direction) {
  const float cos_max_horizontal_delta_angle = std::cos(
      MathUtil::DegToRad(90.0 - params.max_horizontal_delta_angle_degrees()));
...
void ClassifyPlane(const PlaneClassificationConfigProto& config,
  ...
  if (IsHorizontalPlaneWithOffset(config.floor_params(), *planar_region,
    ...
    *planar_region->mutable_plane_class() = PlanarRegion<>::PlaneClass::Floor;
    ...
    *planar_region->mutable_plane_class() =
        PlanarRegion<>::PlaneClass::Coffeetable;
  } else if (IsWall(config.wall_params(), *planar_region,
                    robot_normal_direction)) {
    // Is the plane a wall?
    *planar_region->mutable_plane_class() = PlanarRegion<>::PlaneClass::Wall;
  } else {
    // Default
    *planar_region->mutable_plane_class() = PlanarRegion<>::PlaneClass::Unknown;
  }
}
